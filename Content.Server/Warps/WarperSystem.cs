...
public class WarperSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<WarperComponent, DragDropTargetEvent>(OnDragDrop);
    ...
    private void OnActivate(EntityUid uid, WarperComponent component, ActivateInWorldEvent args)
    {
        if (component.Dungeon)
        {
            ...
            var map = _map.LoadMap(dMap, "/Dungeon14/Maps/Templates/dungeon.yml");
    ...
    private void OnDragDrop(EntityUid uid, WarperComponent component, DragDropTargetEvent args)
    {
        if (args.Handled)
            return;

        args.Handled = true;
        DoWarp(component.Owner, args.User, args.Dragged, component);
    }
}
